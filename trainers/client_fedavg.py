...
def stackoverflow_nwp_generalized_test(net, train_data, test_data):
    ...
    after_loss, after_accuracy, after_correct, after_total = before_loss, before_accuracy, before_correct, before_total
    # print("Accuracy: ", correct, "/", total, " = ", accuracy)
    return before_loss, before_accuracy, after_loss, after_accuracy, before_correct, before_total
...
def synthetic_generalized_test(net, train_data, test_data):
    ...
    after_loss, after_accuracy, after_correct, after_total = 0.0, 0.0, 0, 1
...
def cifar10_generalized_test(net, train_data, test_data):
    ...
    before_loss, before_accuracy, before_correct, before_total = cifar10_test(net, train_data)
    after_loss, after_accuracy, after_correct, after_total = before_loss, before_accuracy, before_correct, before_total

    return before_loss, before_accuracy, after_loss, after_accuracy, after_correct, before_total
...
def cifar100_train(net, train_data, test_data):
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_data:
            ...
            optimizer.zero_grad()
            logits = net(inputs)
            # make_dot(logits, params=dict(list(net.named_parameters()))).render(checkpoint_dir + 'train', format="png")
...
def cifar100_test(net, test_data):
    ...
    with torch.no_grad():
        for inputs, labels in test_data:
            ...
            logits = net(inputs)
            # print(logits)
            # make_dot(logits, params=dict(list(net.named_parameters()))).render(checkpoint_dir + 'test', format="png")
...
def stackoverflow_lr_generalized_test(net, train_data, test_data):
    ...
    after_loss, after_recall, after_correct, after_total = before_loss, before_recall, before_correct, before_total
    # print("Accuracy: ", correct, "/", total, " = ", accuracy)
...
class StackoverflowNWPClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        return float(before_loss), count, {'before_accuracy': float(before_accuracy), 'after_accuracy': float(after_accuracy), 'correct': int(correct), 'misclassified': int(0), 'harmed': bool(False)}
...
class EMNISTClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        return float(before_loss), int(count), {'before_accuracy': float(before_accuracy), 'after_accuracy': float(after_accuracy), 'correct': int(correct), 'misclassified': int(0), 'harmed': bool(False)}
...
class Cifar10Client(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        return float(after_loss), count, {'before_accuracy': float(before_accuracy), 'after_accuracy': float(after_accuracy), 'correct': int(correct), 'misclassified': int(0), 'harmed': bool(False)}
...
class Cifar100Client(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        before_loss, before_accuracy, after_loss, after_accuracy, correct, count = cifar100_generalized_test(self.net, train_data_, test_data_)
        return float(after_loss), int(count), {'before_accuracy': float(before_accuracy), 'after_accuracy': float(after_accuracy), 'correct': int(correct), 'misclassified': int(0), 'harmed': bool(False)}
...
class ShakespeareClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        test_data_ = test_data.create_tf_dataset_for_client(self.cid)
        self.set_parameters(parameters)
        before_loss, before_accuracy, after_loss, after_accuracy, correct, count = shakespeare_generalized_test(self.net, train_data_, test_data_)
        return float(before_loss), count, {'before_accuracy': float(before_accuracy), 'after_accuracy': float(after_accuracy), 'correct': int(correct), 'misclassified': int(0), 'harmed': bool(False)}
