...
class StackoverflowNWPClient(fl.client.NumPyClient):
    ...
    def set_parameters(self, parameters):
        ...
        state_dict = OrderedDict({k: torch.Tensor(v) for k, v in params_dict  if 'linear' in k})
...
class Cifar10Client(fl.client.NumPyClient):
    ...
    def set_parameters(self, parameters):
        ...
        state_dict = OrderedDict({k: torch.Tensor(v) for k, v in params_dict if k in named_params and 'fc' in k})
        self.net.load_state_dict(state_dict, strict=False)
...
class ShakespeareClient(fl.client.NumPyClient):
    ...
    def set_parameters(self, parameters):
        params_dict = zip(self.net.state_dict().keys(), parameters)
        state_dict = OrderedDict({k: torch.Tensor(v) for k, v in params_dict if 'fc' in k})
        self.net.load_state_dict(state_dict, strict=False)
