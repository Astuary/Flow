...
def stackoverflow_nwp_train(net_1, net_2, train_data, test_data):
    criterion = nn.CrossEntropyLoss(ignore_index=0).to(DEVICE)
    optimizer_1 = optim.SGD(net_1.parameters(), lr=lr, momentum=0.9)
    optimizer_2 = optim.SGD(net_2.parameters(), lr=lr, momentum=0.9)

    original_net_1 = copy.deepcopy(net_1)
    original_net_2 = copy.deepcopy(net_2)
    
    net_1.train()
    net_2.train()
    for epoch in range(epochs):
        
        accumulated_loss_1 = 0
        accumulated_loss_2 = 0
        sample_count = 0
        ...
        for tokens, next_tokens in train_data:
            ...
            logits_1 = net_1(tokens_)
            logits_2 = net_2(tokens_)
            loss_1 = criterion(logits_1, next_tokens_)
            loss_2 = criterion(logits_2, next_tokens_)
            accumulated_loss_1 += loss_1.item()
            accumulated_loss_2 += loss_2.item()
    ...
    accumulated_loss = 0
    if accumulated_loss_2 > accumulated_loss_1:
        accumulated_loss = accumulated_loss_1
        _, accuracy, _, _ = stackoverflow_nwp_test(net_1, test_data)
        net_2.load_state_dict(original_net_2.state_dict())
    else:
        accumulated_loss = accumulated_loss_2
        _, accuracy, _, _ = stackoverflow_nwp_test(net_2, test_data)
        net_1.load_state_dict(original_net_1.state_dict())
...
def stackoverflow_nwp_generalized_test(net_1, net_2, train_data, test_data):
    criterion = nn.CrossEntropyLoss(ignore_index=0).to(DEVICE)
    optimizer_1 = optim.SGD(net_1.parameters(), lr=lr, momentum=0.9)
    optimizer_2 = optim.SGD(net_2.parameters(), lr=lr, momentum=0.9)
    ...
    for epoch in range(epochs):
        ...
        accumulated_loss_1, accumulated_loss_2 = 0.0, 0.0
        ...
        if epoch == 0:
            if accumulated_loss_1 < accumulated_loss_2:
                before_net = copy.deepcopy(net_1).to(DEVICE)
            else:
                before_net = copy.deepcopy(net_2).to(DEVICE)

    if accumulated_loss_1 < accumulated_loss_2:
        net = copy.deepcopy(net_1).to(DEVICE)
    else:
        net = copy.deepcopy(net_2).to(DEVICE)
...
def emnist_train(net_1, net_2, train_data, test_data):
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_data:
            ...
            optimizer_1.zero_grad()
            optimizer_2.zero_grad()
            logits_1 = net_1(inputs)
            logits_2 = net_2(inputs)
    ...
    if accumulated_loss_2 > accumulated_loss_1:
        accumulated_loss = accumulated_loss_1
        _, accuracy, _, _ = emnist_test(net_1, test_data)
        net_2.load_state_dict(original_net_2.state_dict())
    else:
        accumulated_loss = accumulated_loss_2
        _, accuracy, _, _ = emnist_test(net_2, test_data)
        net_1.load_state_dict(original_net_1.state_dict())
...
def emnist_generalized_test(net_1, net_2, train_data, test_data):
    criterion = nn.CrossEntropyLoss().to(DEVICE)
    optimizer_1 = optim.SGD(net_1.parameters(), lr=lr, momentum=0.9)
    optimizer_2 = optim.SGD(net_2.parameters(), lr=lr, momentum=0.9)
...
def cifar10_train(net_1, net_2, train_data, test_data):
    criterion = nn.CrossEntropyLoss().to(DEVICE)
    optimizer_1 = optim.SGD(net_1.parameters(), lr=lr, momentum=0.9)
    optimizer_2 = optim.SGD(net_2.parameters(), lr=lr, momentum=0.9)
    ...
    for epoch in range(epochs):
        ...
        accumulated_loss_1, accumulated_loss_2 = 0, 0
        sample_count = 0
    ...
    if accumulated_loss_2 > accumulated_loss_1:
        accumulated_loss = accumulated_loss_1
        _, accuracy, _, _ = cifar10_test(net_1, test_data)
        net_2.load_state_dict(original_net_2.state_dict())
    else:
        accumulated_loss = accumulated_loss_2
        _, accuracy, _, _ = cifar10_test(net_2, test_data)
        net_1.load_state_dict(original_net_1.state_dict())
...
def cifar10_generalized_test(net_1, net_2, train_data, test_data):
    criterion = nn.CrossEntropyLoss().to(DEVICE)
    optimizer_1 = optim.SGD(net_1.parameters(), lr=lr, momentum=0.9)
    optimizer_2 = optim.SGD(net_2.parameters(), lr=lr, momentum=0.9)
...
def cifar100_train(net_1, net_2, train_data, test_data):
    criterion = nn.CrossEntropyLoss().to(DEVICE)
    optimizer_1 = optim.SGD(net_1.parameters(), lr=lr, momentum=0.9)
    optimizer_2 = optim.SGD(net_2.parameters(), lr=lr, momentum=0.9)
...
def cifar100_generalized_test(net_1, net_2, train_data, test_data):
    criterion = nn.CrossEntropyLoss().to(DEVICE)
    optimizer_1 = optim.SGD(net_1.parameters(), lr=lr, momentum=0.9)
    optimizer_2 = optim.SGD(net_2.parameters(), lr=lr, momentum=0.9)
...
def shakespeare_train(net_1, net_2, train_data, test_data):
    criterion = nn.CrossEntropyLoss(ignore_index=0).to(DEVICE)
    optimizer_1 = optim.SGD(net_1.parameters(), lr=lr, momentum=0.9)
    optimizer_2 = optim.SGD(net_2.parameters(), lr=lr, momentum=0.9)
    ...
    for epoch in range(epochs):
        ...
        for chars, next_chars in train_data:
            ...
            optimizer_1.zero_grad()
            optimizer_2.zero_grad()
            logits_1 = net_1(chars)
            logits_2 = net_2(chars)
            loss_1 = criterion(logits_1, next_chars)
            loss_2 = criterion(logits_2, next_chars)
            accumulated_loss_1 += loss_1.item()
            accumulated_loss_2 += loss_2.item()
    ...
    if accumulated_loss_2 > accumulated_loss_1:
        accumulated_loss = accumulated_loss_1
        _, accuracy, _, _ = shakespeare_test(net_1, test_data)
        net_2.load_state_dict(original_net_2.state_dict())
    else:
        accumulated_loss = accumulated_loss_2
        _, accuracy, _, _ = shakespeare_test(net_2, test_data)
        net_1.load_state_dict(original_net_1.state_dict())
...
def shakespeare_generalized_test(net_1, net_2, train_data, test_data):
    criterion = nn.CrossEntropyLoss(ignore_index=0).to(DEVICE)
    optimizer_1 = optim.SGD(net_1.parameters(), lr=lr, momentum=0.9)
    optimizer_2 = optim.SGD(net_2.parameters(), lr=lr, momentum=0.9)
...
class StackoverflowNWPClient(fl.client.NumPyClient):
    ...
    def __init__(self, cid, net_1, net_2):# -> None:
        self.net_1 = net_1
        self.net_2 = net_2
        self.cid = cid.strip()

    def get_parameters(self, config):
        list_1 = [val.cpu().numpy() for _, val in self.net_1.state_dict().items()]
        list_2 = [val.cpu().numpy() for _, val in self.net_2.state_dict().items()]
        return list_1 + list_2

    def set_parameters(self, parameters):
        parameters_1 = parameters[:int(len(parameters)/2)]
        parameters_2 = parameters[int(len(parameters)/2):]
        params_dict_1 = zip(self.net_1.state_dict().keys(), parameters_1)
        params_dict_2 = zip(self.net_2.state_dict().keys(), parameters_2)
        ...
        self.net_1.load_state_dict(state_dict_1, strict=True)
        self.net_2.load_state_dict(state_dict_2, strict=True)
    ...
    def fit(self, parameters, config):
        ...
        loss, count = stackoverflow_nwp_train(self.net_1, self.net_2, train_data_, test_data_)
    ...
    def evaluate(self, parameters, config):
        ...
        test_data_ = test_data.create_tf_dataset_for_client(self.cid)
        self.set_parameters(parameters)
        before_loss, before_accuracy, after_loss, after_accuracy, correct, count, harmed, w_g_true_w_p_true, w_g_true_w_p_false, w_g_false_w_p_true, w_g_false_w_p_false = stackoverflow_nwp_generalized_test(self.net_1, self.net_2, train_data_, test_data_)
...
class EMNISTClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        loss, count = emnist_train(self.net_1, self.net_2, train_data_, test_data_)
        return self.get_parameters(config), count, {'loss': float(loss)}
    ...
    def evaluate(self, parameters, config):
        ...
        before_loss, before_accuracy, after_loss, after_accuracy, correct, count, harmed, w_g_true_w_p_true, w_g_true_w_p_false, w_g_false_w_p_true, w_g_false_w_p_false = emnist_generalized_test(self.net_1, self.net_2, train_data_, test_data_)
        return float(after_loss), count, {'before_accuracy': float(before_accuracy), 'after_accuracy': float(after_accuracy), 'correct': int(correct), 'harmed': bool(harmed), 'w_g_true_w_p_true': int(w_g_true_w_p_true), 'w_g_true_w_p_false': int(w_g_true_w_p_false), 'w_g_false_w_p_true': int(w_g_false_w_p_true), 'w_g_false_w_p_false': int(w_g_false_w_p_false)}
...
class Cifar10Client(fl.client.NumPyClient):
    ...
    def set_parameters(self, parameters):
        ...
        params_dict_1 = zip(self.net_1.state_dict().keys(), parameters_1)
        params_dict_2 = zip(self.net_2.state_dict().keys(), parameters_2)
        named_params_1 = [n for n, p in self.net_1.named_parameters()]
        named_params_2 = [n for n, p in self.net_2.named_parameters()]
        ...
        self.net_1.load_state_dict(state_dict_1, strict=False)
        self.net_2.load_state_dict(state_dict_2, strict=False)
    ...
    def fit(self, parameters, config):
        ...
        loss, count = cifar10_train(self.net_1, self.net_2, train_data_, test_data_)
        return self.get_parameters(config), count, {'loss': float(loss)}
    ...
    def evaluate(self, parameters, config):
        ...
        before_loss, before_accuracy, after_loss, after_accuracy, correct, count, harmed, w_g_true_w_p_true, w_g_true_w_p_false, w_g_false_w_p_true, w_g_false_w_p_false = cifar10_generalized_test(self.net_1, self.net_2, train_data_, test_data_)
        return float(after_loss), count, {'before_accuracy': float(before_accuracy), 'after_accuracy': float(after_accuracy), 'correct': int(correct), 'harmed': bool(harmed), 'w_g_true_w_p_true': int(w_g_true_w_p_true), 'w_g_true_w_p_false': int(w_g_true_w_p_false), 'w_g_false_w_p_true': int(w_g_false_w_p_true), 'w_g_false_w_p_false': int(w_g_false_w_p_false)}
...
class Cifar100Client(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        loss, count = cifar100_train(self.net_1, self.net_2, train_data_, test_data_)
        return self.get_parameters(config), count, {'loss': float(loss)}
    ...
    def evaluate(self, parameters, config):
        ...
        before_loss, before_accuracy, after_loss, after_accuracy, correct, count, harmed, w_g_true_w_p_true, w_g_true_w_p_false, w_g_false_w_p_true, w_g_false_w_p_false = cifar100_generalized_test(self.net_1, self.net_2, train_data_, test_data_)
        return float(after_loss), count, {'before_accuracy': float(before_accuracy), 'after_accuracy': float(after_accuracy), 'correct': int(correct), 'harmed': bool(harmed), 'w_g_true_w_p_true': int(w_g_true_w_p_true), 'w_g_true_w_p_false': int(w_g_true_w_p_false), 'w_g_false_w_p_true': int(w_g_false_w_p_true), 'w_g_false_w_p_false': int(w_g_false_w_p_false)}
...
class ShakespeareClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        test_data_ = test_data.create_tf_dataset_for_client(self.cid)
        self.set_parameters(parameters)
        loss, count = shakespeare_train(self.net_1, self.net_2, train_data_, test_data_)
        return self.get_parameters(config), count, {'loss': float(loss)}
    ...
    def evaluate(self, parameters, config):
        ...
        test_data_ = test_data.create_tf_dataset_for_client(self.cid)
        self.set_parameters(parameters)
        before_loss, before_accuracy, after_loss, after_accuracy, correct, count, harmed, w_g_true_w_p_true, w_g_true_w_p_false, w_g_false_w_p_true, w_g_false_w_p_false = shakespeare_generalized_test(self.net_1, self.net_2, train_data_, test_data_)
        return float(after_loss), count, {'before_accuracy': float(before_accuracy), 'after_accuracy': float(after_accuracy), 'correct': int(correct), 'harmed': bool(harmed), 'w_g_true_w_p_true': int(w_g_true_w_p_true), 'w_g_true_w_p_false': int(w_g_true_w_p_false), 'w_g_false_w_p_true': int(w_g_false_w_p_true), 'w_g_false_w_p_false': int(w_g_false_w_p_false)}
...
def client_fn(cid: str):# -> fl.client.Client:
    ...
    if DATASET == 'stackoverflow_nwp':
        net_1 = StackoverflowNWPNet(vocab_size + 4, embedding_size, hidden_size, num_layers).to(DEVICE)
        net_2 = StackoverflowNWPNet(vocab_size + 4, embedding_size, hidden_size, num_layers).to(DEVICE)
        return StackoverflowNWPClient(cid, net_1, net_2)
    elif DATASET == 'synthetic':
        net_1 = SyntheticNet(input_dim, hidden_dim, output_dim).to(DEVICE)
        net_2 = SyntheticNet(input_dim, hidden_dim, output_dim).to(DEVICE)
        return SyntheticClient(cid, net_1, net_2)
    elif DATASET == 'emnist':
        net_1 = EMNISTNet().to(DEVICE)
        net_2 = EMNISTNet().to(DEVICE)
        return EMNISTClient(cid, net_1, net_2)
    elif DATASET == 'cifar10':
        net_1 = CifarResNet(num_classes=10).to(DEVICE)
        net_2 = CifarResNet(num_classes=10).to(DEVICE)
        return Cifar10Client(cid, net_1, net_2)
    elif DATASET == 'cifar100':
        net_1 = CifarResNet(num_classes=100).to(DEVICE)
        net_2 = CifarResNet(num_classes=100).to(DEVICE)
        return Cifar100Client(cid, net_1, net_2)
    elif DATASET == 'shakespeare':
        net_1 = ShakespeareNet(embedding_size, vocab_size, hidden_size).to(DEVICE)
        net_2 = ShakespeareNet(embedding_size, vocab_size, hidden_size).to(DEVICE)
        return ShakespeareClient(cid, net_1, net_2)
    elif DATASET == 'stackoverflow_lr':
        net_1 = StackoverflowLRNet(word_vocab_size, tag_vocab_size).to(DEVICE)
        net_2 = StackoverflowLRNet(word_vocab_size, tag_vocab_size).to(DEVICE)
        return StackoverflowLRClient(cid, net_1, net_2)
...
def load_parameters_from_disk():# -> fl.common.Parameters:
    ...
    net_1.load_state_dict(checkpoint['net_1_state_dict'])
    net_2.load_state_dict(checkpoint['net_2_state_dict'])
    print(Fore.YELLOW + f"Loading model weights from round #{checkpoint['round']}" + Fore.WHITE)
    
    list_1 = [val.cpu().numpy() for _, val in net_1.state_dict().items()]
    list_2 = [val.cpu().numpy() for _, val in net_2.state_dict().items()]

    return fl.common.ndarrays_to_parameters(list_1 + list_2), rounds - checkpoint['round']
