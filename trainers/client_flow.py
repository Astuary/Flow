...
from models.stackoverflow_nwp_rnn_dynamic_v4 import StackoverflowNet as StackoverflowNWPNet
from models.synthetic_fc import SyntheticNet
from models.emnist_cnn_dynamic_v4 import EMNISTNet
from models.cifar100_resnet_dynamic_v4 import resnet18 as CifarResNet
from models.shakespeare_rnn_dynamic_v4 import ShakespeareNet
...
def stackoverflow_nwp_train(net, train_data, test_data):
    ...
    for epoch in range(epochs):
        ...
        for tokens, next_tokens in train_data.shard(num_shards=2, index=1):
            tokens_, next_tokens_ = torch.from_numpy(tokens.numpy()).to(DEVICE), torch.from_numpy(next_tokens.numpy()).to(DEVICE)
            ...
            optimizer.zero_grad()
            logits, probs = net(tokens_, mode = 'personalized')
            loss = criterion(logits, next_tokens_) + lambda_ * (torch.mean(-torch.log(probs[:, :, :, 1] ** 2 + epsilon)))
            loss.backward()
            optimizer.step()
            ...
            optimizer.zero_grad()
            logits, _ = net(tokens_, mode = 'global')
            loss = criterion(logits, next_tokens_)
...
def stackoverflow_nwp_test(net, test_data):
    ...
    with torch.no_grad():
        for tokens, next_tokens in test_data:
            tokens_, next_tokens_ = torch.from_numpy(tokens.numpy()).to(DEVICE), torch.from_numpy(next_tokens.numpy()).to(DEVICE)
            logits, _ = net(tokens_, mode = 'personalized', hard_decision = True)
...
def stackoverflow_nwp_generalized_test(net, train_data, test_data):
    ...
    with torch.no_grad():
        for tokens, next_tokens in test_data:
            ...
            before_logits, _ = before_net(tokens_, mode = 'global', hard_decision = True)
            after_logits, _ = net(tokens_, mode = 'personalized', hard_decision = True)
...
def emnist_train(net, train_data, test_data):
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_data.shard(num_shards=2, index=1):
            labels = labels.numpy()
            inputs, labels = torch.from_numpy(inputs.numpy()).to(DEVICE), torch.from_numpy(labels).type(torch.LongTensor).to(DEVICE)
            ...
            optimizer.zero_grad()
            logits, probs = net(inputs, mode = 'personalized')
            loss = criterion(logits, labels) + lambda_ * (torch.mean(-torch.log(probs[:, 1] ** 2 + epsilon)))
            loss.backward()
            optimizer.step()
            ...
            optimizer.zero_grad()
            logits, _ = net(inputs, mode = 'global')
            loss = criterion(logits, labels)
            accumulated_loss += loss.item()
...
def emnist_test(net, test_data):
    ...
    with torch.no_grad():
        for inputs, labels in test_data:
            ...
            logits, _ = net(inputs, mode = 'personalized', hard_decision = True)
...
def emnist_generalized_test(net, train_data, test_data):
    ...
    with torch.no_grad():
        ...
        for inputs, labels in test_data:
            ...
            before_logits, _ = before_net(inputs, mode = 'global')
            after_logits, _ = net(inputs, mode = 'personalized', hard_decision = True)
...
def cifar10_generalized_test(net, train_data, test_data):
    ...
    optimizer = optim.SGD(net.parameters(), lr=lr, momentum=0.9)
    # before_net = copy.deepcopy(net).to(DEVICE)
    # before_optimizer = optim.SGD(before_net.parameters(), lr=lr, momentum=0.9)
    ...
    #         before_optimizer.zero_grad()
    #         logits = before_net(inputs, mode = 'global')
    #         loss = criterion(logits, labels)
    #         loss.backward()
...
def shakespeare_train(net, train_data, test_data):
    ...
    for epoch in range(epochs):
        ...
        for chars, next_chars in train_data.shard(num_shards=2, index=1):
            chars, next_chars = torch.from_numpy(chars.numpy()).to(DEVICE), torch.from_numpy(next_chars.numpy()).to(DEVICE)
            optimizer.zero_grad()
            logits, probs = net(chars, mode = 'personalized')
            loss = criterion(logits, next_chars) + lambda_ * (torch.mean(-torch.log(probs[:, 1] ** 2 + epsilon)))
            loss.backward()
            optimizer.step()
        ...
        for chars, next_chars in train_data:
            chars, next_chars = torch.from_numpy(chars.numpy()).to(DEVICE), torch.from_numpy(next_chars.numpy()).to(DEVICE)
            optimizer.zero_grad()
            logits, _ = net(chars, mode = 'global')
            loss = criterion(logits, next_chars)
            accumulated_loss += loss.item()
...
def shakespeare_test(net, test_data):
    ...
    with torch.no_grad():
        for chars, next_chars in test_data:
            chars, next_chars = torch.from_numpy(chars.numpy()).to(DEVICE), torch.from_numpy(next_chars.numpy()).to(DEVICE)
            logits, _ = net(chars, mode = 'personalized', hard_decision = True)
...
def shakespeare_generalized_test(net, train_data, test_data):
    ...
    with torch.no_grad():
        for chars, next_chars in test_data:
            ...
            before_logits, _ = before_net(chars, mode = 'global')
            after_logits, _ = net(chars, mode = 'personalized', hard_decision = True)
...
class EMNISTClient(fl.client.NumPyClient):
    ...
    def set_parameters(self, parameters):
        ...
        #     if 'local' in k:
        #         state_dict[k] = copy.deepcopy(state_dict[k.replace('local', 'global')])
        self.net.load_state_dict(state_dict, strict=True)
...
def client_fn(cid: str):# -> fl.client.Client:
    ...
    if DATASET == 'stackoverflow_nwp':
        net = StackoverflowNWPNet(vocab_size + 4, embedding_size, hidden_size, sequence_length, num_layers).to(DEVICE)
...
if __name__  == '__main__':
    ...
    METHOD = 'FlowV4' # Dynamically routed
    ...
    if DATASET == 'stackoverflow_nwp':
        ...
        # max_count_cid, min_count_cid = None, None
        # max_count, min_count = 25, 999999
        # for idx, cid in enumerate(clients):
        #     print(idx, end='\r')
        #     t = train_data.create_tf_dataset_for_client(cid.strip())
        #     sample_count = 0
        #     for tokens, _ in t:
        #         sample_count += tokens.shape[0]
        #     print(cid, sample_count)
        # #     if max_count < sample_count:
        # #         max_count = sample_count
        # #         max_count_cid = cid
        # #     if min_count > sample_count:
        # #         min_count = sample_count
        # #         min_count_cid = cid
        # #     if idx >= 10000:
        # #         break
        # # print(max_count_cid, min_count_cid)
        ...
        hidden_size = hyperparameters['hidden_size']
        vocab_size = hyperparameters['vocab_size']
        train_data, test_data = shakespeare_dataloader.get_federated_datasets(train_client_batch_size=batch_size)
        clients = range(total_clients)

        ## client 375 - 5 samples
        ## client 1003 - 1 samples

        # max_count_cid, min_count_cid = None, None
        # max_count, min_count = 3, 10
        # for idx, cid in enumerate(clients):
        #     print(idx, end='\r')
        #     t = train_data.create_tf_dataset_for_client(train_data.client_ids[cid])
        #     sample_count = 0
        #     for tokens, _ in t:
        #         sample_count += tokens.shape[0]
        #     if max_count < sample_count:
