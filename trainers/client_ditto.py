...
def stackoverflow_nwp_train(net, train_data, test_data):
    ...
    for epoch in range(epochs):
        ...
        for tokens, next_tokens in train_data:
            ...
            for n, p in zip(net.parameters(), personalized_net.parameters()):
                l2_norm += torch.sum(torch.pow(n-p, 2))

            if l2_norm.item() != 0.0:
                l2_norm = torch.sqrt(l2_norm)

            loss += ((lambda_ / torch.tensor(2.0)) * l2_norm)
...
def synthetic_generalized_test(net, train_data, test_data):
    ...
    for epoch in range(epochs):
        ...
        loss, accuracy, correct, total = synthetic_test(personalized_net, test_data)
...
def stackoverflow_lr_generalized_test(net, train_data, test_data):
    ...
    for epoch in range(epochs):
        ...
        loss, recall, correct, total = stackoverflow_lr_test(personalized_net, test_data)
...
class StackoverflowLRClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        # print(before_loss, before_accuracy, after_loss, after_accuracy, correct, count)
...
def load_parameters_from_disk():# -> fl.common.Parameters:
    model_file_name = checkpoint_dir + 'model_' + METHOD + '_' + str(num_clients) + '_' + str(total_clients) + '_' + str(rounds) + '_' + str(epochs) + '_' + str(lr).replace('.', '_') + '.pth'
...
if __name__  == '__main__':
    ...
    if not (DATASET in ['stackoverflow_nwp', 'stackoverflow_lr', 'cifar10', 'movielens', 'synthetic', 'amazon', 'emnist', 'cifar100', 'shakespeare']):
