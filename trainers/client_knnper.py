...
from models.stackoverflow_nwp_rnn_memoization import StackoverflowNet as StackoverflowNWPNet
from models.synthetic_fc import SyntheticNet
from models.emnist_cnn_memoization import EMNISTNet
from models.cifar100_resnet_memoization import resnet18 as CifarResNet
from models.shakespeare_rnn_memoization import ShakespeareNet
...
def stackoverflow_nwp_generalized_test(net, train_data, test_data):
    ...
    for tokens, next_tokens in train_data:
        ...
        logits, embs = net(tokens_)
        for i in range(embs.shape[0]):
            datastore[count] = torch.argmax(logits[i].data, dim =1)
            count += 1

            if embeddings != None:
                embeddings = torch.vstack((embeddings, embs[i].data[None, :]))
            else:
                embeddings = embs[i].data[None, :]
    ...
    with torch.no_grad():
        for tokens, next_tokens in test_data:
            ...
            before_logits, input_embeddings = net(tokens_)
            dist = torch.norm(embeddings - input_embeddings, dim=1, p=None)
            knn = dist.topk(knn_neighbors_count, largest=False)
            knn_logits = torch.zeros(before_logits.shape, device = DEVICE)
            
            # print(knn.indices.data.shape)
            # print(datastore[0].shape)
            # print(knn_logits.shape)
            # input('gr')
            for idx, key in enumerate(knn.indices.data):
                # for j in range(vocab_size):
                knn_logits[0, datastore[key.item()], datastore[key.item()]] += torch.exp(-knn.values[idx])#.item()
                                 
            after_logits, _ = net(tokens_)
            
            before_loss += criterion(before_logits, next_tokens_).item()
            after_loss += criterion((1 - lambda_) * after_logits + (lambda_) * knn_logits, next_tokens_).item()
            
            _, before_predicted = torch.max(before_logits, 1)
            _, after_predicted = torch.max((1 - lambda_) * after_logits + (lambda_) * knn_logits, 1)
...
def emnist_generalized_test(net, train_data, test_data):
    ...
    for inputs, labels in train_data:
        ...
        logits, embs = net(inputs)
        
        for i in range(embs.shape[0]):
            datastore[count] = torch.argmax(logits[i].data)
            count += 1

            if embeddings != None:
                embeddings = torch.vstack((embeddings, embs[i].data))
            else:
                embeddings = embs[i].data
    ...
    with torch.no_grad():
        ...
        for inputs, labels in test_data:
            ...
            before_logits, input_embeddings = net(inputs)
            ...
            knn_logits = torch.zeros(before_logits.shape, device = DEVICE)
            for idx, key in enumerate(knn.indices.data):
                knn_logits[0, datastore[key.item()]] += torch.exp(-knn.values[idx])#.item()
            
            after_logits, _ = net(inputs)
            
            before_loss += criterion(before_logits, labels).item()
            after_loss += criterion((1 - lambda_) * after_logits + (lambda_) * knn_logits, labels).item()
...
def shakespeare_generalized_test(net, train_data, test_data):
    ...
    with torch.no_grad():
        for chars, next_chars in test_data:
            ...
            before_logits, input_embeddings = net(chars)
            dist = torch.norm(embeddings - input_embeddings, dim=1, p=None)
            knn = dist.topk(knn_neighbors_count, largest=False)
            knn_logits = torch.zeros(before_logits.shape, device = DEVICE)
            for idx, key in enumerate(knn.indices.data):
                for j in range(vocab_size):
                    knn_logits[0, j, datastore[key.item()]] += torch.exp(-knn.values[idx])#.item()
                      
            after_logits, _ = net(chars)
            
            before_loss += criterion(before_logits, next_chars).item()
            after_loss += criterion((1 - lambda_) * after_logits + (lambda_) * knn_logits, next_chars).item()
...
if __name__  == '__main__':
    ...
    knn_neighbors_count = hyperparameters['knn_neighbor_count']
    ...
    if DATASET == 'stackoverflow_nwp':
        ...
        train_data, test_data = stackoverflow_nwp_dataloader.get_federated_datasets(vocab_size, sequence_length, train_client_batch_size=batch_size, test_client_batch_size=1)
        ...
        train_data, test_data = emnist_dataloader.get_federated_datasets(train_client_batch_size=batch_size, test_client_batch_size=1)
        with open(dataset_dir + 'available_train_clients.txt', 'r') as f:
            clients = f.readlines()

    elif DATASET == 'cifar100':
        dirichlet_parameter = hyperparameters["dirichlet_parameter"]
        train_data, test_data = cifar100_dataloader.get_federated_datasets(dirichlet_parameter=dirichlet_parameter, total_clients=total_clients, test_client_batch_size=1)
        ...
        hidden_size = hyperparameters['hidden_size']
        vocab_size = hyperparameters['vocab_size']
        train_data, test_data = shakespeare_dataloader.get_federated_datasets(test_client_batch_size=1)
        clients = range(total_clients)
        ...
        dirichlet_parameter = hyperparameters["dirichlet_parameter"]
        train_data, test_data = cifar10_dataloader.get_federated_datasets(dirichlet_parameter=dirichlet_parameter, total_clients=total_clients, test_client_batch_size=1)
        clients = range(total_clients)
    ...
    print(Fore.RED + "Available Device: " + str(DEVICE) + ", Count: " + str(AVAILABLE_GPUS) + Fore.WHITE)
    ...
    # loss, accuracy, correct, count = stackoverflow_nwp_generalized_test(net, train_data_, test_data_)
    ...
    # net = EMNISTNet().to(DEVICE)
    # print(emnist_generalized_test(net, train_data_, test_data_))
    ...
    # net = CifarResNet().to(DEVICE)
    # print(cifar10_generalized_test(net, train_data_, test_data_))
