...
class StackoverflowNet(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_size, hidden_size, sequence_length, num_layers):
        ...
        self.client_embedding = nn.Embedding(200000, 20)
    ...
    def forward(self, x, cid, mode = 'personalized', hard_decision = False):
        ...
        if mode == 'local':  
            ...
            for i in range(sequence_length):
                ...
                probabilities = torch.softmax(self.policy_net(cid_emb, lstm = True), dim = 2)#.unsqueeze(1)
                if hard_decision:
                ...
                if hidden_states == None:
                    ...
                    probs_hidden = torch.cat([probs_hidden, probabilities], dim=1)
                    hidden_states = torch.cat([hidden_states, global_hidden[:, None, :]], dim=1)
            ...
            p1, p2 = self.policy_net(cid_emb, lstm = False)
            ...
            x = probabilities[:, :, 0].unsqueeze(1).expand_as(global_x) * global_x + probabilities[:, :, 1].unsqueeze(1).expand_as(local_x) * local_x
            ...
            logits =  global_logits * probabilities[:, :, 0].unsqueeze(1).expand_as(global_logits) + local_logits * probabilities[:, :, 1].unsqueeze(1).expand_as(local_logits)
...
class StackoverflowPolicyNet(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_size, hidden_size, sequence_length) -> None:
        ...
        self.lstm_linear_1 = nn.Linear(20, 150)
        self.lstm_linear_2 = nn.Linear(150, 100)
        self.lstm_linear_3 = nn.Linear(100, 60)
        self.lstm_linear_4 = nn.Linear(60, 25)
        self.lstm_linear_exit = nn.Linear(25, 2)

        self.linear_1 = nn.Linear(20, 100)
        self.linear_2 = nn.Linear(100, 60)
        self.linear_2_exit = nn.Linear(60, 2)
        self.linear_3 = nn.Linear(60, 25)
        self.linear_3_exit = nn.Linear(25, 2)
