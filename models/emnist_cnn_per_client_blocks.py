...
class EMNISTNet(nn.Module):
    ...
    def forward(self, x, alpha):
        ...
        global_x = self.relu(global_x)
        local_x = self.relu(local_x)
        global_x = self.max_pooling(global_x)
        local_x = self.max_pooling(local_x)
        global_x = self.dropout_1(global_x)
        local_x = self.dropout_1(local_x)
        global_x = self.flatten(global_x)
        local_x = self.flatten(local_x)
        ...
        global_x = self.relu(global_x)
        local_x = self.relu(local_x)
        global_x = self.dropout_2(global_x)
        local_x = self.dropout_2(local_x)
