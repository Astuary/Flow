...
class ShakespeareNet(nn.Module):
    ...
    def forward(self, input_seq, mode = 'personalized', hard_decision = False):
        ...
        if mode == 'local':
            ...
            for i in range(sequence_length):
                ...
                probabilities_hidden_1 = torch.softmax(self.policy_net(torch.cat([embedding.contiguous().view(-1, embedding_dim), global_hidden_1, global_cell_1, local_hidden_1, local_cell_1], 1), lstm = True), dim=1).unsqueeze(1)
                ...
                probabilities_hidden_2 = torch.softmax(self.policy_net(torch.cat([embedding.contiguous().view(-1, embedding_dim), global_hidden_2, global_cell_2, local_hidden_2, local_cell_2], 1), lstm = True), dim=1).unsqueeze(1)
...
class ShakespearePolicyNet(nn.Module):
    ...
    def __init__(self, embedding_size, vocab_size, hidden_size) -> None:
        ...
        self.lstm_linear_1 = nn.Linear(embedding_size + 4 * hidden_size, 500)
        ...
        self.linear_1 = nn.Linear(90 * 2, 100)
        self.linear_1_exit = nn.Linear(100, 2)
