...
class EMNISTNet(nn.Module):
    ...
    def __init__(self, only_digits=False):
        ...
        self.embeddings = torch.nn.Embedding(3400, 12)
    ...
    def forward(self, x, cid, mode = 'local', hard_decision = False):
        x = torch.squeeze(x)
        if len(x.shape) == 2:
        ...
        if mode == 'local':
            ...
            cid_emb = self.embeddings(cid)
            p1, p2, p3, p4 = self.prob_policy_net(cid_emb)
            ...
            probs_hidden = torch.cat([p1[:, None], p2[:, None], p3[:, None], p4[:, None]], dim=1)
                
        return x, probs_hidden
