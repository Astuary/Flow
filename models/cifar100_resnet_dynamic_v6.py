...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=100, group_norm=2):
        ...
        self.client_embedding = nn.Embedding(500, 9)
    ...
    def forward(self, x, cid, mode = 'personalized', hard_decision = False):
        ...
        if mode == 'local':
            ...
            p1, p2, p3, p4, p5, p6 = self.prob_policy_net(cid_emb)

            if hard_decision:
                p1, p2, p3, p4, p5, p6 = torch.round(p1), torch.round(p2), torch.round(p3), torch.round(p4), torch.round(p5), torch.round(p6) 
            # print(p1.shape, p2.shape, p3.shape, p4.shape, p5.shape, p6.shape)

            global_x = self.global_conv1(x)
            local_x = self.local_conv1(x)

            x = global_x * p1[:, 0].unsqueeze(1).unsqueeze(2).unsqueeze(3).expand_as(global_x) + local_x * p1[:, 1].unsqueeze(1).unsqueeze(2).unsqueeze(3).expand_as(local_x)
            ...
            x = global_x * p2[:, 0].unsqueeze(1).unsqueeze(2).unsqueeze(3).expand_as(global_x) + local_x * p2[:, 1].unsqueeze(1).unsqueeze(2).unsqueeze(3).expand_as(local_x)
            
            global_x = self.global_layer2(x)
            local_x = self.local_layer2(x)
            x = global_x * p3[:, 0].unsqueeze(1).unsqueeze(2).unsqueeze(3).expand_as(global_x) + local_x * p3[:, 1].unsqueeze(1).unsqueeze(2).unsqueeze(3).expand_as(local_x)

            global_x = self.global_layer3(x)
            local_x = self.local_layer3(x)
            x = global_x * p4[:, 0].unsqueeze(1).unsqueeze(2).unsqueeze(3).expand_as(global_x) + local_x * p4[:, 1].unsqueeze(1).unsqueeze(2).unsqueeze(3).expand_as(local_x)

            global_x = self.global_layer4(x)
            local_x = self.local_layer4(x)
            x = global_x * p5[:, 0].unsqueeze(1).unsqueeze(2).unsqueeze(3).expand_as(global_x) + local_x * p5[:, 1].unsqueeze(1).unsqueeze(2).unsqueeze(3).expand_as(local_x)
            ...
            x = global_x * p6[:, 0].unsqueeze(1).expand_as(global_x) + local_x * p6[:, 1].unsqueeze(1).expand_as(local_x)

            probs = torch.cat([p1[:, :, None], p2[:, :, None], p3[:, :, None], p4[:, :, None], p5[:, :, None], p6[:, :, None]], dim=2)
        return x, probs
...
class PolicyNet(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.linear_1 = nn.Linear(9, 200)
        self.linear_2 = nn.Linear(200, 150)
        self.linear_3 = nn.Linear(150, 100)
        self.linear_4 = nn.Linear(100, 50)
        self.linear_5 = nn.Linear(50, 25)
        self.linear_6 = nn.Linear(25, 2)

        self.linear_1_exit = nn.Linear(200, 2)
        self.linear_2_exit = nn.Linear(150, 2)
        self.linear_3_exit = nn.Linear(100, 2)
        self.linear_4_exit = nn.Linear(50, 2)
        self.linear_5_exit = nn.Linear(25, 2)
