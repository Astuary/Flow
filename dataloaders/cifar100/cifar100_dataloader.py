...
TEST_EXAMPLES = 10000
# Number of training examples per class: 50,000 / 100.
TRAIN_EXAMPLES_PER_LABEL = int(TRAIN_EXAMPLES / NUM_CLASSES)
# Number of test examples per class: 10,000 / 100.
TEST_EXAMPLES_PER_LABEL = int(TEST_EXAMPLES / NUM_CLASSES)


def load_cifar100_federated(
    ...
    Args:
      dirichlet_parameter: Parameter of Dirichlet distribution. Each client
        ...
        partitioning.
      total_clients: The number of clients the examples are going to be partitioned
        on.
    Returns:
...
def create_preprocess_fn(
    ...
    def preprocess_fn(dataset):
        return (
            ...
            .map(image_map_fn, num_parallel_calls=num_parallel_calls)
            .batch(batch_size)
        )
...
def get_federated_datasets(
    ...
    train_shuffle_buffer_size: int = 1,
    ...
    if train_client_epochs_per_round < 1:
        raise ValueError("train_client_epochs_per_round must be a positive integer.")
    ...
    cifar_train, cifar_test = load_cifar100_federated(dirichlet_parameter=dirichlet_parameter, total_clients=total_clients)
